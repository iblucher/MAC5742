...
void *thread_task(void * _data) {
	struct Thread_Data *thread_data = _data;
	for(int i = thread_data->idx_start; i < thread_data->idx_end; i++)
		if(advance(thread_data->st, thread_data->data[i], thread_data->data[i+1]))
			thread_data->count++;
...
int task(char * data, long len, char * search) {
	pthread_t *threads_array = malloc(sizeof(pthread_t)*threads);
	struct Thread_Data *thread_data = malloc(sizeof(struct Thread_Data)*threads);

	int blk = len / threads;
	int offset = 0;
	for(int i = 0; i < threads; i++) {
		thread_data[i].data = data;
		thread_data[i].len = blk;
		thread_data[i].search = search;
		thread_data[i].st = createState(search);
		thread_data[i].idx_start = offset;
		thread_data[i].idx_end = offset + blk;
		thread_data[i].count = 0;

		offset += blk;
	}
	thread_data[threads - 1].idx_end += len % threads;

	// cria as thread
	for(int i = 0; i < threads; i++) {
		pthread_create(threads_array + i, NULL, thread_task, thread_data + i);
	}
